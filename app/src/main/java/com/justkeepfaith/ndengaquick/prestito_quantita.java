...
public class prestito_quantita extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        applitittle.setText("Hallo " + name + ", Ihr aktuelles Kreditlimit beträgt €" +(applimit));
    ...
    private void requestloan() {
        ...
        if (wenteramount.isEmpty()) {
            enteramount.setError("Kann nicht leer sein");
            return;
        }
        if (!wenteramount.isEmpty()) {
            ...
            if (ienteramount > wapplimit) {
                enteramount.setError("Ihr Limit ist €" + applimit);
                return;
            }
            if (ienteramount < 5) {
                enteramount.setError("Minimum ist €5");
                return;
            }
        }
        if (!Conectivity.isConnectingToInternet(prestito_quantita.this)) {
            Toast.makeText(prestito_quantita.this, "Bitte überprüfe deine Internetverbindung", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        progressDialog.setMessage("Sparen...");
